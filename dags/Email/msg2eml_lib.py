...
import sys
def ignore_unraisable(err):
    exc = err.exc_value
    if isinstance(exc, AttributeError) and err.object and err.object.__class__.__name__ == "CompoundFileMiniStream":
        # 跳过此错误，不输出
        return 
    # 其它异常调用默认处理
...
import compoundfiles
from compoundfiles import streams

# 保存原始close方法以防需要
orig_close = streams.CompoundFileMiniStream.close

def safe_close(self):
    """改进的 close 方法，避免 None.close 异常"""
    try:
        # 只有在 _file 未被清空时才调用其 close()
        if getattr(self, "_file", None):
            self._file.close()
    except Exception:
        pass  # 忽略close过程中的任何异常
    finally:
        self._file = None  # 确保_file被置None
# 应用 monkey patch
streams.CompoundFileMiniStream.close = safe_close
...
def load_message_stream(entry, is_top_level, doc):
  ...
  # 1) 纯文本 BODY
  if 'BODY' in props and props['BODY']:
      body = props['BODY']
      # 用 8bit，方便肉眼阅读与全文检索
      msg.set_content(body, subtype="plain", charset="utf-8", cte="8bit")
      has_body = True

  # 2) HTML（PR_HTML = 0x1013）
  if not has_body and 'HTML' in props and props['HTML']:
      ...
      if html_body.strip():
          try:
              ...
              text_body = "此邮件正文为 HTML。"
          msg.set_content(text_body, subtype="plain", charset="utf-8", cte="8bit")
          msg.add_alternative(html_body, subtype="html", charset="utf-8", cte="quoted-printable")
          has_body = True

  # 3) RTF 压缩（PR_RTF_COMPRESSED）
  if not has_body and 'RTF_COMPRESSED' in props and props['RTF_COMPRESSED']:
      import compressed_rtf
      rtf = props['RTF_COMPRESSED']
      try:
          rtf_data = compressed_rtf.decompress(rtf)
          try:
              rtf_blob = io.BytesIO(rtf_data)
              ...
              if html_body and html_body.strip():
                  try:
                      ...
                      text_body = "此邮件正文为 HTML（由 RTF 提取）。"
                  msg.set_content(text_body, subtype="plain", charset="utf-8", cte="8bit")
                  msg.add_alternative(html_body, subtype="html", charset="utf-8", cte="quoted-printable")
                  has_body = True
          except Exception:
              # 退路：附上 RTF 文件，正文给个占位
              msg.set_content("<无法解析 RTF 内容，见附件 body.rtf>", subtype="plain", charset="utf-8", cte="8bit")
              msg.add_attachment(rtf_data, maintype="text", subtype="rtf", filename="body.rtf")
              has_body = True
      except Exception:
          pass

  if not has_body:
      msg.set_content("<no message body>", subtype="plain", charset="utf-8", cte="8bit")
...
if __name__ == "__main__":
    if len(sys.argv) <= 1:
        ...
        data = sys.stdin.buffer.read()       # ← 二进制
        bio = io.BytesIO(data)                # ← 有 seek()/tell()
        msg = load(bio)
        sys.stdout.buffer.write(msg.as_bytes())
    else:
        for fn in sys.argv[1:]:
            with open(fn, "rb") as f:         # ← 二进制打开
                msg = load(f)
